...
const debugMode = (function () {
  try {
    return /\bdebug=1\b/.test(window.location.search) || window.localStorage.getItem('debug') === '1';
  } catch (e) {
    return /\bdebug=1\b/.test(window.location.search);
  }
})();
...
if (slideshowContainer) {
  ...
  function updateGhostPreviews() {
    ...
    const prevSrc = prevSlideImg ? prevSlideImg.getAttribute('src') : null;
    const prev2Src = prev2SlideImg ? prev2SlideImg.getAttribute('src') : null;
    const nextSrc = nextSlideImg ? nextSlideImg.getAttribute('src') : null;
    const next2Src = next2SlideImg ? next2SlideImg.getAttribute('src') : null;
    if (ghostPrevImg && prevSrc) ghostPrevImg.src = prevSrc;
    if (ghostPrev2Img && prev2Src) ghostPrev2Img.src = prev2Src;
    if (ghostNextImg && nextSrc) ghostNextImg.src = nextSrc;
    if (ghostNext2Img && next2Src) ghostNext2Img.src = next2Src;
  }
...
setThemeColorMeta(htmlElement.getAttribute('data-theme') === 'dark' ? 'dark' : 'light');
...
const RFQ_STORAGE_PREFIX = 'rfq_';
const RFQ_STORAGE_META_KEY = 'rfq_meta';
const RFQ_STORAGE_TTL_MS = 7 * 24 * 60 * 60 * 1000;

function isStorageAvailable() {
  try {
    const testKey = '__rfq_test__';
...
function getRfqMeta() {
  ...
  try {
    const raw = localStorage.getItem(RFQ_STORAGE_META_KEY);
    if (!raw) return null;
...
function setRfqMeta(partial) {
  if (!isStorageAvailable()) return;
  const existing = getRfqMeta() || {};
  const next = Object.assign({}, existing, partial, { ts: Date.now(), v: 1 });
  try {
    localStorage.setItem(RFQ_STORAGE_META_KEY, JSON.stringify(next));
  } catch (_) {}
}
...
function clearFormData() {
  ...
  formFields.forEach(fieldId => {
    try {
      localStorage.removeItem(RFQ_STORAGE_PREFIX + fieldId);
    } catch (_) {}
  });
  try {
    localStorage.removeItem(RFQ_STORAGE_META_KEY);
  } catch (_) {}
}

function clearIfExpired() {
  const meta = getRfqMeta();
  if (!meta || typeof meta.ts !== 'number') return;
  if (Date.now() - meta.ts > RFQ_STORAGE_TTL_MS) clearFormData();
}
...
function loadFormData() {
  ...
  formFields.forEach(fieldId => {
    ...
    let savedValue = null;
    try {
      savedValue = localStorage.getItem(RFQ_STORAGE_PREFIX + fieldId);
    } catch (_) {}
...
function saveFormData() {
  ...
  formFields.forEach(fieldId => {
    ...
    try {
      if (value) {
        hasAnyValue = true;
        localStorage.setItem(RFQ_STORAGE_PREFIX + fieldId, value);
      } else {
        localStorage.removeItem(RFQ_STORAGE_PREFIX + fieldId);
      }
    } catch (_) {}
